    ...
    )
{
    ...
    gcCover->gcTable           = pCodeInfo->GetGCTable();
...
public:
    ...
    GCCoverageRangeEnumerator (ICodeManager *pCodeManager, GCTable pvGCTable, BYTE *codeStart, SIZE_T codeSize)
    {
        ...
        BOOL   fZapped)
{
...
#ifdef _TARGET_AMD64_
    GCCoverageRangeEnumerator rangeEnum(codeMan, gcTable, codeStart, codeSize);

    GcInfoDecoder safePointDecoder(gcTable, (GcInfoDecoderFlags)0, 0);
    ...
    */
    ...
    while (cur < codeEnd)
    {
        ...
        if (codeMan->IsInPrologOrEpilog((cur - codeStart) + (DWORD)regionOffsetAdj, gcTable.Info, NULL)) {
            *cur = INTERRUPT_INSTR;
        }
#endif
...
void DoGcStress (PCONTEXT regs, MethodDesc *pMD)
{
    ...
    /* are we in a prolog or epilog?  If so just test the unwind logic
       ...
       GC safe points */
    if (gcCover->codeMan->IsInPrologOrEpilog(offset, gcCover->gcTable.Info, NULL))
    {
