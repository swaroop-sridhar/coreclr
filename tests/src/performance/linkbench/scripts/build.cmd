...
call %__dotnet% publish -c Release -r win7-x64
if errorlevel 1 set ExitCode=1 
...
REM Run Linker
call %__dotnet% %__IlLinkDll% -t -c link -a @%AssetDir%\Roslyn\RoslynRoots.txt -x %AssetDir%\Roslyn\RoslynRoots.xml -l none -out ..\Linked
if errorlevel 1 set ExitCode=1 
popd
exit /b
